"""
Master Publication Artifacts Generator
Parses all experimental CSV datasets and generates:
1. Camera-Ready LaTeX Tables (.tex) for Sections 6, 7, and 8.
2. Publication-Quality Vector PDF/PNG Figures (IEEE/ACM compliant, 300 DPI).
"""
...
def generate_latex_tables(csv_dir="paper_outputs/csv_datasets", tables_dir="paper_outputs/tables"):
    os.makedirs(tables_dir, exist_ok=True)
    print("[*] Generating Camera-Ready LaTeX Tables...")

    # Table 7: Scalability Performance Table
    sc_df = pd.read_csv(os.path.join(csv_dir, "monte_carlo_scalability_results.csv"))
    with open(os.path.join(tables_dir, "table_performance_metrics.tex"), "w") as f:
        ...
        f.write("\\caption{Monte Carlo Scalability Evaluation across 50 Independent Runs per Scale.}\n")
    ...
    # Table 8: Gas Comparison Table
    gas_df = pd.read_csv(os.path.join(csv_dir, "evm_gas_benchmarks.csv"))
    ...
    print("[+] LaTeX tables successfully generated in: paper_outputs/tables/")


def generate_publication_figures(csv_dir="paper_outputs/csv_datasets", fig_dir="paper_outputs/figures"):
    os.makedirs(fig_dir, exist_ok=True)
    print("[*] Rendering High-Resolution Vector Figures (.pdf / .png)...")

    # Figure 13: Byzantine Resilience (P_capture & Fork Rate vs. f)
    byz_df = pd.read_csv(os.path.join(csv_dir, "byzantine_resilience_results.csv"))
    ...
    plt.axvline(x=33.3, color="black", linestyle=":", label="Theoretical BFT Bound ($f=33.3\%$)")
    plt.xlabel("Byzantine Adversary Fraction $f$ (%)")
    plt.ylabel("Governance Capture Probability $P(Capture)$")
    ...
    # Figure 14: Gas Scaling Comparison
    gas_df = pd.read_csv(os.path.join(csv_dir, "evm_gas_benchmarks.csv"))
    plt.figure(figsize=(6.5, 4.0))
    ...
    plt.plot(gas_df["Committee_Size_m"], gas_df["PBFT_ViewChange_Gas"], "s--", color="#d62728", label="PBFT View-Change $\\mathcal{O}(m^2)$", lw=1.5)
    ...
    # Figure 15: Global Sobol Sensitivity Indices
    sobol_df = pd.read_csv(os.path.join(csv_dir, "sobol_sensitivity_results.csv"))
    ...
    print("[+] Publication vector figures generated in: paper_outputs/figures/")
